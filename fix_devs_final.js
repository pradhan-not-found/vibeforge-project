...
const buildAcrossIdx = page.indexOf('<div className="w-full bg-surface py-[120px]">');
if (buildAcrossIdx !== -1) {
    const nextSectionIdx = page.indexOf('<footer', buildAcrossIdx);
    if (nextSectionIdx !== -1) {
        console.log("Replacing from", buildAcrossIdx, "to", nextSectionIdx);
        const toReplace = page.substring(buildAcrossIdx, nextSectionIdx);
        // Ensure toReplace is just this section
        console.log("Length of section to replace:", toReplace.length);
...
`;
        page = page.substring(0, buildAcrossIdx) + devsSection + page.substring(nextSectionIdx);
        fs.writeFileSync('src/app/page.tsx', page);
        console.log("Successfully replaced Build Across Industries with Meet the Devs!");
    } else {
        console.log("Footer not found after build across industries!");
    }
} else {
    console.log("Build across industries not found!");
}
